...
if __name__ == "__main__":
    ...
    # 替换为你实际的 CSV 路径
    POLDER_CSV_PATH = '/media/amers/SSD_part1/whx/ResNet_forPolder/polder_aeronet_matched_dataset_withPrior_9pixel.csv'
    
    try:
        dpc_df = pd.read_csv(POLDER_CSV_PATH)
        print(f"成功读取 DPC CSV 匹配文件，共 {len(dpc_df)} 条记录。")
    except FileNotFoundError:
        print(f"[错误] 找不到 DPC CSV 文件: '{POLDER_CSV_PATH}'")
        exit()
    ...
    output_name = "polder_retrieval_results_9pixel.csv"
