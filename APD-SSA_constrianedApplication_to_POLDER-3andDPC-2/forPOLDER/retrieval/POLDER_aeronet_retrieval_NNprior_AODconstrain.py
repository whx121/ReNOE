# --- START OF FILE POLDER_aeronet_retrieval_NNprior_AODconstrain.py ---
...
# 导入自定义模块
from moudle.ResNet_RTModel_pytorch import DNNModel
import moudle.moudle_OE as OEfunc
from moudle.predict_forAOD import predict_AOD_AAOD_SSA as predict_AOD
from moudle.DNNModel import DNNModel as AerosolDNNModel
...
class RetrievalConfig:
    ...
    # === AERONET 真值约束容差 ===
    # sigma_aeronet_aod: float = 0.1
    # sigma_aeronet_ssa: float = 0.1
    sigma_aeronet_aod_rel: float = 0.15  
    sigma_aeronet_aod_min: float = 0.05
    
    sigma_aeronet_ssa_rel: float = 0.15
    sigma_aeronet_ssa_min: float = 0.05
    ...
    def _init_bounds(self):
        self.state_bounds = [
            (0.000001, 1.5),  # vc_BB
            (0.000001, 1.5),  # vc_Urban
            (0.000001, 1.5),  # vc_Ocean
            (0.000001, 2),    # vc_Dust
            (0.5, 7),        # ALH
            (0.001, 0.8),    # iso_0.443
            (0.001, 0.8),    # iso_0.490
    ...
    def _optimize_single(self, data, r_obs, model_dict, features_scaler, init_state) -> object:
        ...
        def residual_function(sv):
            ...
            if valid_aod or valid_ssa:
                ...
                x_tensor = torch.from_numpy(x_scaled_aerosol).float().to(device)
                ...
                if valid_aod:
                    dynamic_sigma_aod = max(truth_aod * self.config.sigma_aeronet_aod_rel, self.config.sigma_aeronet_aod_min)
                    res_aod = np.array([(pred_aod - truth_aod) / dynamic_sigma_aod])
                    residuals = np.concatenate([residuals, res_aod])
                
                if valid_ssa:
                    dynamic_sigma_ssa = max(truth_ssa * self.config.sigma_aeronet_ssa_rel, self.config.sigma_aeronet_ssa_min)
                    res_ssa = np.array([(pred_ssa - truth_ssa) / dynamic_sigma_ssa])
                    residuals = np.concatenate([residuals, res_ssa])
            print("residuals", res_aod, res_ssa)
            return residuals
        ...
        def jac_function(sv):
            ...
            if valid_aod or valid_ssa:
                ...
                x_tensor = torch.tensor(x_scaled_aerosol, dtype=torch.float32, requires_grad=True, device=device)
                ...
                if valid_aod:
                    dynamic_sigma_aod = max(truth_aod * self.config.sigma_aeronet_aod_rel, self.config.sigma_aeronet_aod_min)
                    J_val = np.vstack([J_val, full_jac_row_aod / dynamic_sigma_aod])
                
                if valid_ssa:
                    dynamic_sigma_ssa = max(truth_ssa * self.config.sigma_aeronet_ssa_rel, self.config.sigma_aeronet_ssa_min)
                    J_val = np.vstack([J_val, full_jac_row_ssa / dynamic_sigma_ssa])
...
def parse_polder_row_to_data_obs(row: pd.Series) -> np.ndarray:
    """
    POLDER 专用数据解析：无需定标系数，11个角度，包含 1020nm
    注意：已将所有观测强度 I 转换为 TOA，以匹配正向模型的输出。
    """
    data_obs_list = []
    for ang_idx in range(11):  
        ...
        TOA443 = I443 / cos_sza if I443 > 0 else 0.0
        
        I565 = row.get(f'I565_ang{ang_idx}', 0.0)
        TOA565 = I565 / cos_sza if I565 > 0 else 0.0
        
        I1020 = row.get(f'I1020_ang{ang_idx}', 0.0)
        TOA1020 = I1020 / cos_sza if I1020 > 0 else 0.0
        
        def calc_toa_dolp(wl):
        ...
        TOA490, DOLP490 = calc_toa_dolp('490')
        TOA670, DOLP670 = calc_toa_dolp('670')
        TOA865, DOLP865 = calc_toa_dolp('865')
        
        angle_obs = [sza, vza, phi, TOA443, TOA490, DOLP490, TOA565, TOA670, DOLP670, TOA865, DOLP865, TOA1020]
...
def build_vc_dnn_features(row: pd.Series, obs_time: pd.Timestamp) -> np.ndarray:
    """
    POLDER 专用特征提取：11个角度 * 12个变量 + 4个基础特征 = 136维
    注意：所有强度 I 均已转换为 TOA = I / cos(sza)
    """
    # 基础特征 (4个)
    features = [row['elev']/1000]
    
    for ang_idx in range(11): # POLDER 11个角度
        sza = row.get(f'sza_ang{ang_idx}')
        if pd.isna(sza) or sza <= 0:
            features.extend([0.0] * 12) # 填充 12 个 0
            continue
        ...
        def get_stokes_toa(wl):
            I = row.get(f'I{wl}_ang{ang_idx}')
            Q = row.get(f'Q{wl}_ang{ang_idx}')
            U = row.get(f'U{wl}_ang{ang_idx}')
            I_val = I if pd.notna(I) else 0.0
            Q_val = Q if pd.notna(Q) else 0.0
            U_val = U if pd.notna(U) else 0.0
            
            # 计算 TOA 和 DOLP (注意 DOLP 的分母依然是 I_val，除以 cos 约掉了)
            TOA = I_val / cos_sza if I_val > 0 else 0.0
            DOLP = np.sqrt(Q_val**2 + U_val**2) / I_val if I_val > 1e-6 else 0.0
            return TOA, DOLP

        TOA490, DOLP490 = get_stokes_toa('490')
        TOA670, DOLP670 = get_stokes_toa('670')
        TOA865, DOLP865 = get_stokes_toa('865')
...
def process_multi_angle_data_improved(data_obs, elev, lon, lat, prior_path, config: RetrievalConfig,
    ...
    try:
        ...
        prior_values = {
            ...
            'iso_1.02': np.clip(iso_1020, 0.005, 0.8), # 恢复 iso_1020
...
if __name__ == "__main__":
    ...
    # === POLDER 匹配数据路径 ===
    POLDER_CSV_PATH = '/media/amers/SSD_part1/whx/ResNet_forPolder/polder_aeronet_match_code/polder_aeronet_matched_data_ready_for_retrieval_withprior_part.csv' 
    
    try:
        polder_df = pd.read_csv(POLDER_CSV_PATH)
        print(f"成功读取 POLDER CSV 匹配文件，共 {len(polder_df)} 条记录。")
    except FileNotFoundError:
        print(f"[错误] 找不到 POLDER CSV 文件: '{POLDER_CSV_PATH}'")
        exit()

    config = RetrievalConfig()

    output_path = r'/media/amers/SSD_part1/whx/ResNet_forPolder/'
    os.makedirs(output_path, exist_ok=True)
    output_name = "polder_retrieval_results_ResNOE_part.csv"

    # ==============================================================================
    # 加载 正向辐射传输模型(DNN) - POLDER 6波段版
    # ==============================================================================
    print("\n===== 加载多波段正向 DNN 模型 =====")
    ...
    # ==============================================================================
    # 加载 气溶胶体积浓度预测模型(VC Model) - POLDER 版
    # ==============================================================================
    print("\n===== 加载动态先验 VC DNN 模型 =====")
    VC_MODEL_PATH = '/media/amers/SSD_part1/whx/ResNet_forPolder/dynamic_prior/vc_best_model_polder.pth' 
    ...
    # POLDER 版输入维度为 
    vc_model = ResidualDNN(133, 6, layers_config=[1024, 512, 256, 128]).to(device)   
    ...
    print("\n===== 开始解析 POLDER 数据并预测动态先验 =====")
    for index, row in tqdm(polder_df.iterrows(), total=len(polder_df), desc="Parsing & Predicting"):
        try:
            obs_time = pd.to_datetime(row['match_datetime'])
            ...
            # 使用 POLDER 专用解析函数
            data_obs = parse_polder_row_to_data_obs(row)
            ...
            if len(data_obs) > 0:
                ...
                elev_list.append(row['elev']/1000) 
                lon_list.append(row['lon'])
                lat_list.append(row['lat'])
                times_list.append(row['match_datetime'])
