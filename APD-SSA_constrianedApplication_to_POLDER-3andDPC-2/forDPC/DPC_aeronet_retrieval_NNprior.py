...
# ==============================================================================
# 1. 定义气溶胶先验预测网络模型 (放在 RetrievalConfig 定义的前面)
# ==============================================================================
...
def build_vc_dnn_features(row: pd.Series, obs_time: pd.Timestamp) -> np.ndarray:
    """
    为体积浓度(VC)预测网络提取 136 维特征
    顺序: DOY, lat, lon, elev, [sza, vza, phi, I443, I490, DOLP490, I565, I670, DOLP670, I865, DOLP865] * 12角度
    """
    cal_coeffs = {
        '490P': 1, '565': 1, 
        '670P': 1, '865P': 1
    ...
    # 基础特征 (4个)
    features = [row['elev']]#[obs_time.dayofyear, row['lat'], row['lon'], row['elev']]
    
    # 角度特征 (12个角度 * 11个变量 = 132个)
    for ang_idx in range(12):
        sza = row.get(f'sza_ang{ang_idx}')
        if pd.isna(sza) or sza <= 0:
            # 如果该角度无观测，用 0 填充 11 个变量
...
#         prior_values = {
#             'iso_0.443': np.clip(iso_443, 0.001, 0.5),
#             'iso_0.49': np.clip(iso_490, 0.001, 0.5),
#             'iso_0.565': np.clip(iso_565, 0.001, 0.6),
...
#         elif param == 'o3':
#             non_state_matrix[:, i] = 320  
#         elif param == 'h2o':
#             non_state_matrix[:, i] = 0.5 
...
def process_multi_angle_data_improved(data_obs, elev, lon, lat, prior_path, config: RetrievalConfig,
    ...
    try:
        ...
        print(f"先验数据获取失败: {e}，使用默认值")
        prior_values = {}  # 改为 {} 防止下面 update 报错
...
def update_config_with_prior(config: RetrievalConfig, prior_values: Dict[str, float]) -> RetrievalConfig:
    """使用先验值更新配置，确保初始值在边界内，并激活气溶胶动态先验"""
    import copy
    ...
    for i, param in enumerate(new_config.state_vector_list):
        if param in prior_values:
            ...
            elif param == 'k2': margin = 0.1
            elif param == 'BPDF': margin = 1
            elif param == 'ALH': margin = 1
            # elif param.startswith('vc_'): 
            #     #print(f"⚡ [先验更新] 参数 '{param}' 的先验值: {value:.4f}，激活动态先验约束")
            #     margin = 0.1  # 体积浓度：根据预测值给一个动态搜索范围，不要给太大以免乱跑
            #     # ==== 核心修改 ====
            #     # 动态把先验类型改为 model，激活正规权重，替代原本无效的 guess
            #     new_config.prior_type[param] = "model"
            #     # 根据 parameter base 也设置一下，防兼容 _get_prior_weights
            #     new_config.prior_type[param.split('_')[0]] = "model" 
            #     new_config.prior_sigma[param] = 0.15  # 赋予约束的不确定度
            else:
                margin = 0.5
            ...
            # 如果为0，设为0.2，并确保边界包含0.2
            # if init_value <= 0:
            #     init_value = 0.2
            #     lower = min(lower, init_value * 0.9)
            #     upper = max(upper, init_value * 1.1)
...
# 最终检查：确保所有初始值都在边界内
    for i in range(len(new_config.init_state)):
        ...
        original_val = new_config.init_state[i] # 记录原始值
...
def parallel_pixel_inversion_improved(config: RetrievalConfig, data_obs_list: List,
                                      ...
                                      dynamic_prior_list: List[Dict], # ======= 新增 =======
                                      output_path: str, output_name: str,
    ...
    results = Parallel(n_jobs=n_jobs, backend='loky')(
        delayed(process_single_pixel_improved)(
            ...
            config, model_dict, features_scaler, priori_flag, 
            dynamic_prior_list[i] # ======= 传入当前像元的动态先验 =======
        )
        for i in tqdm(range(n_pixels), desc="Processing pixels")
    )
...
if __name__ == "__main__":
    ...
    output_name = "dpc_retrieval_results_withprior.csv"
    ...
    vc_model = ResidualDNN(133, 4, layers_config=[1024, 512, 256, 128]).to(device)   #133维是不带doy lat lon的
    ...
    for index, row in tqdm(dpc_df.iterrows(), total=len(dpc_df), desc="Parsing & Predicting"):
        try:
            ...
            if len(data_obs) > 0:
                ...
                # 保证物理意义(非负)
                y_pred[y_pred < 0] = 0.0 
                #print(f"第 {index} 行数据预测结果: {y_pred}")
                # 3. 映射到反演配置的名字: ['BC_vol', 'Urban_vol', 'SeaSalt_vol', 'Dust_vol']
                vc_bb    = y_pred[0] if y_pred[0] >= 0.0001 else 0.15
                vc_urban = y_pred[1] if y_pred[1] >= 0.0001 else 0.15
                vc_ocean = y_pred[2] if y_pred[2] >= 0.0001 else 0.15
                vc_dust  = y_pred[3] if y_pred[3] >= 0.0001 else 0.15
                ...
                dynamic_prior_list.append(dynamic_vc_prior) # 存入动态先验
    ...
    if data_obs_list:
        batch_results = parallel_pixel_inversion_improved(
            ...
            dynamic_prior_list=dynamic_prior_list, # ==== 传递先验列表 ====
