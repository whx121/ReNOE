...
from retrieval.inversion_code.aeronet.aeronet_inversion_single_pixel_pytorch import (
...
class RetrievalConfig:
    ...
    smoothness_weights: Dict[str, float] = field(default_factory=lambda: {
        "vc_BB": 1, "vc_Urban": 1, "vc_Ocean": 1, "vc_Dust": 1, "ALH": 10,
    ...
    prior_type: Dict[str, str] = field(default_factory=lambda: {
        "vc_BB": "guess", "vc_Urban": "guess", "vc_Ocean": "guess", "vc_Dust": "guess",
        "ALH": "model", "iso": "climatology", "k1": "climatology", "k2": "climatology", "BPDF": "model"
    })

    # 先验不确定性
    prior_sigma: Dict[str, float] = field(default_factory=lambda: {
        "vc_BB": 0.5, "vc_Urban": 0.5, "vc_Ocean": 0.5, "vc_Dust": 0.5,
        "ALH": 0.5, "iso": 0.1, "k1": 0.1, "k2": 0.1, "BPDF": 0.3
    })

    prior_weight_factor: Dict[str, float] = field(default_factory=lambda: {
        "observation": 1.0, "climatology": 1.0, "guess": 1e-12, "model": 1.0
    })
    ...
    def _init_bounds(self):
        """初始化参数边界"""
        self.state_bounds = [
            (0, 0.7), (0, 0.7), (0, 0.7), (1e-5, 0.7), (0.5, 7),
            (0.001, 0.8), (0.001, 0.8), (0.001, 0.8), (0.001, 0.8), (0.005, 0.8), (0.005, 0.8),
            (0.01, 2), (0.01, 2), (0.5, 8)
        ]
    ...
    def _init_state(self):
        ...
        self.init_state = np.array([
            0.1, 0.1, 0.1, 0.1, 3.5, 0.05, 0.06, 0.1, 0.12, 0.3, 0.4, 0.6, 0.4, 4
        ])
...
def process_data_block(block_info: Dict) -> List[Dict]:
    """处理单个大区域数据块的完整流程 (由每个CPU核心执行)"""
    block_id, boundaries = block_info['id'], block_info['boundaries']
    file_path, file_name, prior_path = block_info['file_path'], block_info['file_name'], block_info['prior_path']
    config, model_dict, features_scaler, polder_time = block_info['config'], block_info['model_dict'], block_info[
        'features_scaler'], block_info['polder_time']

    print(
        f"[进程 {block_id}] 开始. 范围: Lat({boundaries[0]:.2f}-{boundaries[1]:.2f}), Lon({boundaries[2]:.2f}-{boundaries[3]:.2f})")

    try:
        # 1. 生成格网并提取数据
        lat_max, lat_min, lon_min, lon_max = boundaries
        resolution = 0.05


        num_lat = int(np.round((lat_max - lat_min) / resolution)) + 1
        num_lon = int(np.round((lon_max - lon_min) / resolution)) + 1

        # 使用 linspace 生成精确的坐标
        lat_range = np.linspace(lat_max, lat_min, num_lat)
        lon_range = np.linspace(lon_min, lon_max, num_lon)
        lons, lats = np.meshgrid(lon_range, lat_range)
        coordinates = create_coordinate_mapping(lons, lats)

        if not coordinates:
            print(f"[进程 {block_id}] 警告: 没有找到有效坐标")
            return []

        all_pixel_data = vectorized_extract_polder_data(file_path, file_name, coordinates)
        data_obs_list = all_pixel_data['data_obs_list']

        if not data_obs_list:
            print(f"[进程 {block_id}] 警告: 矢量化提取后没有有效像元")
            return []

        print(f"[进程 {block_id}] 矢量化提取完成，共 {len(data_obs_list)} 个有效像元")

        # 2. 一次性预处理所有有效像元 - 修复错误处理
        ...
        for i in range(len(data_obs_list)):
            try:
                r_obs, non_state, updated_config = process_multi_angle_data_improved(
                    data_obs_list[i],
                    all_pixel_data['elev_list'][i],
                    all_pixel_data['lon_list'][i],
                    all_pixel_data['lat_list'][i],
                    prior_path,
                    config,
                    True
                )
                ...
                # 验证数据有效性
                if r_obs is None or r_obs.size == 0:
                    print(f"[进程 {block_id}] 警告: 像元 {i} 的观测数据无效")
                    failed_count += 1
                    continue

                if non_state is None or non_state.size == 0:
                    print(f"[进程 {block_id}] 警告: 像元 {i} 的非状态数据无效")
                    failed_count += 1
                    continue

                preprocessed_data['lon'].append(all_pixel_data['lon_list'][i])
                preprocessed_data['lat'].append(all_pixel_data['lat_list'][i])
                preprocessed_data['elev'].append(all_pixel_data['elev_list'][i])
                ...
                failed_count += 1
                print(f"[进程 {block_id}] 错误: 像元 {i} 预处理失败: {str(e)}")
                # 打印更详细的错误信息用于调试
                if failed_count <= 3:  # 只打印前3个错误的详细信息，避免日志过长
                    import traceback
                    print(f"[进程 {block_id}] 详细错误信息: {traceback.format_exc()}")
                continue

        print(f"[进程 {block_id}] 预处理完成: 成功 {len(preprocessed_data['lon'])} 个，失败 {failed_count} 个")

        if not preprocessed_data['lon']:
            print(f"[进程 {block_id}] 错误: 所有像元预处理都失败了")
            return []

        # 3. 将整个块的数据打包成MultiPixelData对象
        ...
        if not result['success']:
            print(f"[进程 {block_id}] 警告: 优化未完全收敛")
...
def debug_single_pixel_processing(data_obs, elev, lon, lat, prior_path, config):
    """调试单个像元处理过程的辅助函数"""
    print("=== 调试单像元处理 ===")
    print(f"坐标: ({lon:.4f}, {lat:.4f}), 高程: {elev:.2f}")
    print(f"观测数据形状: {data_obs.shape if data_obs is not None else 'None'}")
    print(f"先验路径: {prior_path}")
    print(f"配置对象: {type(config)}")

    try:
        # 检查输入数据
        if data_obs is None:
            raise ValueError("观测数据为None")
        if data_obs.size == 0:
            raise ValueError("观测数据为空")
        if not os.path.exists(prior_path):
            raise FileNotFoundError(f"先验文件不存在: {prior_path}")
        ...
        r_obs, non_state, updated_config = process_multi_angle_data_improved(
            data_obs, elev, lon, lat, prior_path, config, True
        )

        print(f"成功处理: r_obs shape={r_obs.shape}, non_state shape={non_state.shape}")
        return True
...
def main():
    ...
    config.smoothness_weight_global = 0.1

    # 路径配置
    file_path = r"/media/amers/ssd_1t/whx/ResNet_code/retrieval/test_data/"
    file_name = 'POLDER3_L1B-BG1-088222M_2008-10-12T05-16-33_V1-01.h5'
    ...
    output_name = f"{polder_time}_large_area_multi_pixel.csv"
    ...
    # ========== 3. 区域划分 ==========
    n_lat_splits = max(1, int(np.sqrt(NUM_PROCESSES)))
    n_lon_splits = max(1, int(NUM_PROCESSES / n_lat_splits))
    lat_steps = np.linspace(lat_max, lat_min, n_lat_splits + 1)
    lon_steps = np.linspace(lon_min, lon_max, n_lon_splits + 1)

    tasks = []
    for i in range(n_lat_splits):
        for j in range(n_lon_splits):
            tasks.append({
                'id': len(tasks),
                'boundaries': (lat_steps[i], lat_steps[i + 1], lon_steps[j], lon_steps[j + 1]),
                'file_path': file_path, 'file_name': file_name, 'prior_path': prior_path,
                'config': config, 'model_dict': model_dict, 'features_scaler': features_scaler,
                'polder_time': polder_time
            })
    print(f"区域已划分为 {len(tasks)} 个处理块.")
