...
class RetrievalConfig:
    ...
    pixel_window_size: int = 3  # 邻域窗口大小 (3x3 或 2x2)
...
def parallel_multi_pixel_inversion(config: RetrievalConfig, data_obs_list: List,
    ...
    print(windows[1].pixel_indices)
    print(windows[2].pixel_indices)
    print(windows[3].pixel_indices)
...
if __name__ == "__main__":
    ...
    config.use_multi_pixel = True  # 启用多像元约束
    config.pixel_window_size = 2  # 使用3x3窗口
    config.smoothness_weight_global = 0.1  # 调节总体平滑强度

    # 打印配置信息
    print("\n" + "=" * 60)
    print("POLDER-3 气溶胶反演系统配置（多像元约束模式）")
    print("=" * 60)
    print(f"多像元约束: {config.use_multi_pixel}")
    print(f"窗口大小: {config.pixel_window_size}x{config.pixel_window_size}")
    print(f"平滑权重: {config.smoothness_weight_global}")
    print("参数平滑权重:")
    for param, weight in config.smoothness_weights.items():
    ...
    # 输出路径
    output_path_ = r'/media/amers/WHX/NNOE_POLDER/retrieval/aeronet/2006_test_multipixel/'

    # 加载模型
    base_dir = '/media/amers/2E42853942850735/whx/Doctor/NNOE_polder/retrieval/NNpara_polder/new_model_polder/'
        ...
        for file_dict in date_list:
            try:
                ...
                for file_name in os.listdir(file_path):
                    try:
                        print(f'\n开始处理文件：{file_name}')
                        output_name = f"{site}_retrieval_multipixel333.csv"
                        obs_time = file_name.split('_')[2]

                        # 定义研究区域（扩大区域以支持多像元处理）
                        window_margin = 0.05 * config.pixel_window_size  # 根据窗口大小调整
                        ...
                        if data_obs_list:
                            if config.use_multi_pixel:
                                ...
                                batch_results = parallel_multi_pixel_inversion(
                                    ...
                                    features_scaler, obs_time, n_jobs=-1
                                )
                            else:
                                # 使用原始单像元反演（向后兼容）
                                batch_results = parallel_pixel_inversion_improved(
                                    config, data_obs_list, elev_list, lon_list, lat_list,
                                    prior_path, output_path, output_name, model_dict,
                                    features_scaler, obs_time, priori_flag=True, n_jobs=-1
                                )
