...
def parse_args():
    parser = argparse.ArgumentParser(description='POLDER气溶胶反演 - 单站点处理')
    parser.add_argument('--site_index', type=int, required=True,
                       help='指定处理第几个站点（从0开始）')
    parser.add_argument('--sites_csv', type=str,
                       default='/media/amers/WHX/NNOE_POLDER/aeronet/download_aeronet/aeronet_loacation_used_2006.csv',
                       help='站点CSV文件路径')
...
if __name__ == "__main__":
    ...
    print(f"处理站点索引: {args.site_index}")
    print(f"使用线程数: {args.n_jobs}")

    # 设置进程CPU亲和性（可选，因为bash脚本已经用taskset设置了）
    try:
        import psutil
    ...
    if args.site_index >= len(sites_df):
        print(f"[错误] 站点索引 {args.site_index} 超出范围 (0-{len(sites_df) - 1})")
        sys.exit(1)
    ...
    # 输出路径
    output_path_ = r'/media/amers/WHX/NNOE_POLDER/retrieval/aeronet/2006_global_single_0.1_new/'
    ...
    print("\n" + "=" * 50)
    print(f"===== 开始处理站点 {args.site_index}: {site} (Lon: {lon}, Lat: {lat}) =====")
    print("=" * 50)
        ...
        try:
            ...
            for file_name in os.listdir(file_path):
                try:
                    ...
                    # 定义研究区域
                    lat_max, lat_min = lat + 0.1, lat - 0.1 - 0.001
                    lon_min, lon_max = lon - 0.1, lon + 0.1 + 0.001
                    ...
                    if data_obs_list:
                        batch_results = parallel_pixel_inversion_improved(
                            ...
                            priori_flag=True, n_jobs=args.n_jobs  # 使用指定的线程数
                        )
                        # 将结果追加写入文件
            ...
            print(f"站点 {site} 月份 {month} 共处理 {processed_files} 个文件")
    ...
    T_end = time()
    print(f"\n站点 {site} 处理完成，耗时: {(T_end - T_begin) / 60:.2f} 分钟")
    print(f"进程PID {os.getpid()} 完成")