...
def first_retrieval(vc_model,scaler_x_vc,scaler_y_vc, doy, lon, lat, elev, data_obs):
    """
    (升级版) 使用预训练的神经网络模型，反演四种气溶胶组分的体积浓度。
    """
    if vc_model is None or scaler_x_vc is None or scaler_y_vc is None:
        print("错误：模型或Scaler未加载，无法进行预测。")
        return np.array([0.02, 0.01, 0.01, 0.2])

    # --- Step 1: 准备扁平化的特征向量 (与RF版相同) ---
    num_angles_to_extract = 11  #训练动态网络时用了2-12 共11个角度
    ...
    base_features = np.array([doy, lat, lon, elev])  # 确保顺序与训练时一致
    full_feature_vector = np.concatenate([base_features, flattened_angular_features])

    if np.isnan(full_feature_vector).any():
        print("警告：输入特征向量中存在NaN值，由有效观测角度不足引起。")
        return np.array([0.01, 0.01, 0.01, 0.05])

    # --- Step 2: 核心修改 - 使用神经网络进行预测 ---
    try:
        # 2.1 将一维向量转换为 (1, N_features) 的二维数组
        X_input = full_feature_vector.reshape(1, -1)

        # 2.2 使用加载的 scaler_x 对输入特征进行标准化
        X_input_scaled = scaler_x_vc.transform(X_input)

        # 2.3 将标准化的数据转换为PyTorch Tensor
        X_input_tensor = torch.from_numpy(X_input_scaled).float().to(device)

        # 2.4 使用神经网络进行预测 (得到归一化尺度的结果)
        with torch.no_grad():
            predicted_vc_scaled = vc_model(X_input_tensor).cpu().numpy()

        # 2.5 关键：使用 scaler_y 将预测结果逆变换回真实的物理尺度
        predicted_vc = scaler_y_vc.inverse_transform(predicted_vc_scaled)

        # 2.6 确保物理合理性并返回结果
        predicted_vc[predicted_vc < 0] = 0
        # print('神经网络预测结果为：', predicted_vc.flatten())
        return predicted_vc.flatten()  # .flatten() 将 (1, 4) 数组变为 (4,)

    except Exception as e:
        print(f"神经网络预测过程中发生错误: {e}")
        return np.array([0.01, 0.01, 0.01, 0.05])
...
def process_multi_angle_data_improved(vc_model,scaler_x_vc,scaler_y_vc, data_obs, doy, elev, lon, lat, prior_path, config: RetrievalConfig,
    ...
    try:
        ...
        vc_BB, vc_urban, vc_ocean, vc_dust = first_retrieval(vc_model,scaler_x_vc,scaler_y_vc, doy, elev*1000, lon, lat, data_obs) # from km to m


        # 检查先验数据的合理性
        prior_values = {
            'vc_BB':np.clip(vc_BB, 0.0000001, 0.5),
            'vc_urban':np.clip(vc_urban, 0.0000001, 0.5),
            'vc_ocean':np.clip(vc_ocean, 0.0000001, 0.5),
            'vc_dust':np.clip(vc_dust, 0.0000001, 2),  #其实设置到4比较合理 根据MERRA2的分布来看  但当时训练正向时  vc最大设到2
            'iso_0.443': np.clip(iso_443, 0.0001, 0.8),
            'iso_0.49': np.clip(iso_490, 0.0001, 0.8),
            'iso_0.565': np.clip(iso_565, 0.001, 0.8),
            'iso_0.67': np.clip(iso_670, 0.001, 0.8),
            'iso_0.865': np.clip(iso_865, 0.001, 0.8),
            'iso_1.02': np.clip(iso_1020, 0.001, 0.8),
            'k1': np.clip(k1, 0.001, 2),
            'k2': np.clip(k2, 0.001, 2),
            'BPDF': np.clip(BPDF, 0.5, 9),
            'ALH': np.clip(ALH, 0.5, 7)
        }
    except Exception as e:
    ...
    for i, param in enumerate(config.nonstate_vector_list[3:], 3):
        if param == 'dem':
            ...
            non_state_matrix[:, i] = 345  # 可以改为从外部数据源获取
        elif param == 'h2o':
            non_state_matrix[:, i] = 1.2  # 可以改为从外部数据源获取
