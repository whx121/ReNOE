...
class RetrievalConfig:
    ...
    sigma_NN_I: List[float] = field(default_factory=lambda: [0.0007, 0.0008, 0.0007, 0.0008, 0.0008, 0.0006])
    sigma_NN_DOLP: List[float] = field(default_factory=lambda: [0.0, 0.001, 0.0, 0.001, 0.0008, 0.0])

    # 优化设置
    optimization_method: str = 'trf'  # 'trf', 'dogbox', 'lm'
    max_iterations: int = 30  # 最大迭代次数
    xtol: float = 1e-8  # 变量参数容差
    ftol: float = 1e-8  # 函数容差
    gtol: float = 1e-8  # 梯度容差
    ...
    prior_weight_factor: Dict[str, float] = field(default_factory=lambda: {
        ...
        "climatology": 0.7,
        "guess": 0.1,
        "model": 0.3  #尝试缩放VC的影响？  因为地表的量级相对较大
    })
    ...
    def _init_bounds(self):
        ...
        self.state_bounds = [
            ...
            (0.00001, 2),  # vc_Dust
            (0.5, 9),  # ALH
            (0.0001, 0.8),  # iso_0.443
            (0.0001, 0.8),  # iso_0.490
            (0.0001, 0.8),  # iso_0.565
            (0.0001, 0.8),  # iso_0.67
            (0.001, 0.8),  # iso_0.865
            (0.001, 0.8),  # iso_1.02
            (0.001, 2),  # k1
...
def process_multi_pixel_window(window_data: MultiPixelData, config: RetrievalConfig,
    ...
    # 执行多像元联合优化  window——data中存有更新后的config（更新了先验和边界）
...
def organize_pixels_into_windows(data_obs_list: List, doy_list, elev_list: List,
            ...
            if window_pixels:
                ...
                for pixel_idx in window_pixels:
                    ...
                    r_obs, non_state, updated_config = process_multi_angle_data_improved(
                        ...
                        data_obs_list[pixel_idx], doy_list[pixel_idx] , elev_list[pixel_idx],
                    ...
                    #doy_window.append(doy_list[pixel_idx])
...
def parallel_multi_pixel_inversion(config: RetrievalConfig, data_obs_list: List, doy_list: List,
    ...
    # 组织像元为窗口  windows内部更新了config
    windows = organize_pixels_into_windows(
        data_obs_list, doy_list, elev_list, lon_list, lat_list,
    ...
    for window_result in window_results:
        for pixel_result in window_result:
            result_data = {
                ...
                #'initial_state': config.init_state,
                'error': '',
...
def parse_args():
    ...
    parser.add_argument('--smoothness_weight', type=float, default=0.1,
                       help='空间平滑约束权重')
    parser.add_argument('--use_multi_pixel', type=bool, default=False,
                       help='是否启用多像元约束')
...
if __name__ == "__main__":
    ...
    ###  !!不知道为什么sh文件 和 上面的args.use_multi_pixel设为False后仍无法关闭多像元约束（从debug信息来看） ，所以当前直接使用代码中的config.pixel_window_size
    ###  将多像元的开关放在py中  更方便
    ...
    # 输出路径
    output_path_ = r'/media/amers/WHX/NNOE_POLDER/retrieval/aeronet/multi_pixel_test/2006all_test/'
    ...
    VC_MODEL_PATH = '/media/amers/SSD_part1/whx/ResNet_code/dynamic_prior/vc_model_new.pth'  #没有NDVI AOD等参数
    ...
    try:
        #加载先验模型
        print("--- 正在加载神经网络模型和Scalers ---")
        # 实例化模型容器并加载
        # (确保这里的参数与您训练VC模型时一致)
        # VC模型输入不包含NDVI，则为136维
        input_dim_vc = 136
        output_dim_vc = 4
                    ...
                    if data_obs_list:
                        ...
                        if config.use_multi_pixel:
                            ...
                            batch_results = parallel_multi_pixel_inversion(
                                config, data_obs_list, doy_list, elev_list, lon_list, lat_list,
